...
_af_load_current_conf() {
    ...
    while IFS= read -r line; do
        ...
        if [[ "$line" =~ ^[[:space:]]*'set_if_unset APPA_FINO__'([A-Z_]+)[[:space:]]+(.*) ]]; then
            local varname="${match[1]}"
            local raw="${match[2]%% #*}"   # strip trailing inline comment
            raw="${raw%"${raw##*[! ]}"}"    # rtrim spaces
...
_af_select_preset() {
    ...
    print -P "               tomer@myhost:~/projects on main ✔"
    print -P "               ❯"
    print ""
    print -P "  %B[3] verbose%b — Maximum info. Multiline, full paths, timestamps, rich git."
    print -P "               ╭ tomer at myhost in /home/tomer/projects"
    print -P "               │  on ‹main› ✔"
    print -P "               ╰╴❯"
    print ""
...
##########################################
# Generated by appa-fino-configure. Re-run it to reconfigure interactively.
# Uncommented lines override theme defaults.
# Commented-out lines show available options with their default values.
...
## Most variable groups share this convention:
##
##   APPA_FINO__ENABLE_{FEATURE}          — bool: true / false
##   APPA_FINO__{FEATURE}_FOREGROUND_COLOR — color name or 0-255
##   APPA_FINO__{FEATURE}_BACKGROUND_COLOR — color name or 0-255, empty = none
...
##
## Color values: color name (black red green yellow blue magenta cyan white
...
HEADER
        ...
        for group in "${_AF_GROUPS[@]}"; do
            ...
            for var in "${vars[@]}"; do
                ...
                if [[ "$val" == "$default" ]]; then
                    printf '# set_if_unset APPA_FINO__%s %s  # default\n' "$var" "$quoted_val"
                else
                    printf 'set_if_unset APPA_FINO__%s %s\n' "$var" "$quoted_val"
                fi
            done
            printf '\n'
        done
...
appa-fino-configure() {
    ...
    print ""
    print -P "  %F{green}✓ Saved to ${_AF_CONF_FILE} — prompt updated immediately.%f"
    print ""
